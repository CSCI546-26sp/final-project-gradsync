...
sed -i 's/^import \(.*_pb2\) as/from . import \1 as/' packages/comms/src/comms/proto/tensor_service_pb2_grpc.py
...
sed -i 's/^import \(.*_pb2\) as/from . import \1 as/' packages/orchestrator/src/orchestrator/proto/cluster_service_pb2_grpc.py